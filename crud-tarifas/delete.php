...
$sql  = $wpdb->prepare("SELECT descricioServicio FROM " . TABLE_NAME . " WHERE codServicio = %d", $id);
...
<form method="post" action="<?= admin_url('admin-post.php') ?>" class="frm-detail-fruits">
    ...
    <input type="hidden" name="action" value="delete_custom_tarifas">
    <button type="submit" class="btn btn-danger" class="btn btn-danger"><span class="dashicons dashicons-trash"></span>
        Borrar</button>
    <a href="<?= home_url(SLUG_PAGE) ?>"><button type="button" class="btn btn-info"><span
                class="dashicons dashicons-no"></span> Cancelar</button></a>
</form>
